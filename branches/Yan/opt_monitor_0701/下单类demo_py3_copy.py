...
class XtApiCallBack(XtTraderApiCallback):
    ...
    def onRtnLoginStatusWithActKey(
        ...
        self.m_apiClient.onRtnLoginStatusWithActKey(
...
class xtTraderApiClient:
    ...
    def onConnected(self):
        ...
        # 同步登录
        self.testUserLoginSync(
            self.m_strUserName, self.m_strPassword, machineInfo, appid, authcode
        )

    def onConnectError(self, error_msg):
        print(
            "[onConnectError] connect to server: {} failed, error_msg:{}".format(
                self.m_strAddress, error_msg
            )
        )
    ...
    def onRtnLoginStatusWithActKey(
        ...
        if status == EBrokerLoginStatus.BROKER_LOGIN_STATUS_OK:
            print(
                "[onRtnLoginStatusWithActKey] account_id：{}, account_type: {}, 账号登录成功，初始化完成，可以交易".format(
    ...
    def request(self):
        ...
        if self.m_dictAccountID2Key:
            for name in list(self.m_dictAccountID2Key):
    ...
    def stockOrdinaryOrderSync(self, account_id, account_key):
        ...
        if error.isSuccess():
            print("[orderSync]股票普通单, accountId:", account_id, "orderID: ", orderid)
            # 发起撤单
    ...
    def stockOrdinaryOrder(self, account_id, account_key):
        print("[stockOrdinaryOrder]股票普通单, accountId:", account_id)
        orderInfo = COrdinaryOrder()  # 初始化数据，普通单结构体
        orderInfo.m_strAccountID = (
            account_id  # 资金账号，必填参数。不填会被api打回，并且通过onOrder反馈失败
        )
        ...
        orderInfo.m_dSuperPriceRate = 0  # 单笔超价百分比，选填字段。默认为0
        orderInfo.m_nVolume = (
            400  # 报单委托量，必填字段。默认int最大值，填0或不填会被api打回
        )
    ...
    def futureOrdinaryOrder(self, account_id, account_key):
        ...
        orderInfo.m_dSuperPriceRate = 0  # 单笔超价百分比，选填字段。默认为0
        orderInfo.m_nVolume = (
            1  # 报单委托量，必填字段。默认int最大值，填0或不填会被api打回
        )
        ...
        orderInfo.m_eOperationType = (
            EOperationType.OPT_OPEN_LONG
        )  # 报单委托类型。必填字段
        orderInfo.m_strRemark = "cta"
        self.m_nRequestId += 1
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 下单
        print(
            "[futureOrdinaryOrder]期货普通单, accountId:",
            account_id,
            ", requestId:",
            self.m_nRequestId,
        )
    ...
    def algorithmOrder(self, account_id, account_key):
        ...
        orderInfo.m_eSingleVolumeType = (
            EVolumeType.VOLUME_FIX
        )  # 单笔基准量，默认为目标量
        ...
        print(
            "[algorithmOrder]普通算法委托下单, account_id:",
            account_id,
            ", request_id:",
            self.m_nRequestId,
            "pricetype:",
            orderInfo.m_ePriceType,
            "m_eUndealtEntrustRule",
            orderInfo.m_eUndealtEntrustRule,
        )
    ...
    def intelligentAlgorithmOrder(self, account_id, account_key):
        ...
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 开始下单
        print(
            "[intelligentAlgorithmOrder]智能算法委托下单, account_id:",
            account_id,
            ", request_id:",
            self.m_nRequestId,
        )
    ...
    def intelligentAlgorithmOrderSync(self, account_id, account_key):
        ...
        if error.isSuccess():
            print(
                "[orderSync]智能算法委托, accountId:", account_id, "orderID: ", orderid
            )
        else:
    ...
    def externAlgGroupOrder(self, account_id, account_key):
        ...
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 开始下单
        print(
            "[externAlgGroupOrder]主动算法组合单委托下单, account_id:",
            account_id,
            ", request_id:",
            self.m_nRequestId,
        )
    ...
    def externAlgGroupOrderSync(self, account_id, account_key):
        ...
        if error.isSuccess():
            print(
                "[CExternAlgGroupOrder]主动算法组合单委托下单, accountId:",
                account_id,
                "orderID: ",
                orderid,
            )
            ...
            print(
                "[CExternAlgGroupOrder]主动算法组合单下单失败，废单原因： ",
                error.errorMsg(),
            )
    ...
    def AlgGroupOrder(self, account_id, account_key):
        ...
        orderInfo.m_orderParam.m_strAccountID = (
            account_id  # 资金账号 采用多账号下单时，m_strAccountID可不填写
        )
        orderInfo.m_orderParam.m_eOperationType = EOperationType.OPT_BUY  # 委托类型
        ...
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 开始下单
        print(
            "[AlgGroupOrder]智能算法组合单委托下单, account_id:",
            account_id,
            ", request_id:",
            self.m_nRequestId,
        )
    ...
    def stockOrdinaryGroupOrder(self, account_id, account_key):
        ...
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 开始下单
        print(
            "[COrdinaryGroupOrder]普通组合下单, account_id:",
            account_id,
            ", request_id:",
            self.m_nRequestId,
        )
    ...
    def externAlgorithmOrder(self, account_id, account_key):
        ...
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 开始下单
        print(
            "[externAlgorithmOrder]主动算法单下单, account_id:",
            account_id,
            ", request_id:",
            self.m_nRequestId,
        )
    ...
    def externAlgorithmOrderSync(self, account_id, account_key):
        ...
        if error.isSuccess():
            print(
                "[CExternAlgorithmOrder]主动算法单委托下单, accountId:",
                account_id,
                "orderID: ",
                orderid,
            )
            ...
            print(
                "[CExternAlgorithmOrder]主动算法单下单失败，废单原因： ",
                error.errorMsg(),
            )
    ...
    def onRtnOrder(self, data):
        if data.m_eStatus == EOrderCommandStatus.OCS_CHECKING:
            ...
            orderStatus = "默认状态:", data.m_eStatus
        print(
            "[onRtnOrder] 指令编号:",
            data.m_nOrderID,
            ", account_id:",
    ...
    def onRtnOrderDetail(self, data):
        if data.m_eOrderStatus == EEntrustStatus.ENTRUST_STATUS_UNREPORTED:
            ...
            entrust_status = "默认状态->", data.m_eOrderStatus
        print(
            "[onRtnOrderDetail] 指令编号:",
            data.m_nOrderID,
            "，投资备注:",
            data.m_strRemark,
            ", 合同编号:",
    ...
    def onRtnOrderError(self, data):
        if data == None:
            print("[onRtnOrderError], data is None")
        else:
            print(
                "[onRtnOrderError] orderId: ",
                data.m_nOrderID,
                "m_nErrorID: ",
                data.m_nErrorID,
                "m_strErrorMsg: ",
    ...
    def cancelSync(self, order_id, account_key):
        print("[cancel] 撤委托，指令编号:", order_id, "account_key", account_key)
        errno = XtError(0, "")
        self.m_api.cancelSync(order_id, errno, account_key)
        if errno.isSuccess():
            print(
                "[cancel] 撤委托成功，指令编号:", order_id, "account_key", account_key
            )

    # 同步暂停指令（根据指令号）
    def pauseSync(self, order_id):
        errno = XtError(0, "")
        print("[pauseSync] 暂停指令，指令编号:", order_id)
        self.m_api.pauseSync(order_id, errno)
        if errno.isSuccess():
            print("[pauseSync] 暂停指令成功，指令编号:", order_id)

    # 同步恢复指令（根据指令号）
    def resumeSync(self, order_id):
        print("[resumeSync] 恢复指令，指令编号:", order_id)
        errno = XtError(0, "")
        self.m_api.resumeSync(order_id, errno)
        if errno.isSuccess():
            print("[resumeSync] 恢复指令成功，指令编号:", order_id)

    # 同步撤单（根据委托号）
    def cancelOrderSync(self, account_id, order_sysid, market, code, account_key):
        print(
            "[cancel_order] 撤委托，合同编号:", order_sysid, "account_key", account_key
        )
        errno = XtError(0, "")
    ...
    def cancel(self, order_id, account_key):
        print("[cancel_cmd] 撤指令，指令编号:", order_id)
        self.m_nRequestId += 1
    ...
    def onCancel(self, request_id, error):
        if error.isSuccess():
            print("[onCancel] success")
        else:
            print("[onCancel] failed, error_msg: ", error.errorMsg())

    # 撤销指令回调
    def onCancelWithRemark(self, request_id, remark, error):
        if error.isSuccess():
            print("[onCancelWithRemark] success, remark: ", remark)
        else:
            print(
                "[onCancelWithRemark] failed",
                ", remark:",
                remark,
                ", error_msg: ",
                error.errorMsg(),
    ...
    def cancelOrder(self, account_id, order_sysid, market, code, account_key):
        ...
        self.m_nRequestId += 1
        self.m_api.cancelOrder(
            account_id, order_sysid, market, "", self.m_nRequestId, account_key
        )

    # 撤销委托回调, error对应XtError结构
    def onCancelOrder(self, request_id, error):
        if error.isSuccess():
            print("[onCancelOrder] success")
        else:
            print("[onCancelOrder] failed, error_msg: ", error.errorMsg())

    # 获得主推的撤单错误信息，data对应CCancelError结构
    # 撤委托请求被迅投系统风控打回，或者撤销请求被柜台打回，均通过这个接口推送
    def onRtnCancelError(self, data):
        print(
            "[onRtnCancelError]orderId:",
            data.m_nOrderID,
            "m_nErrorID:",
            data.m_nErrorID,
            "m_strErrorMsg:",
    ...
    def future_order(
        ...
        account_key: str,
        price: float,
        super_price_rate: float,
        volume: int,
        market: str,
        code: str,
        price_type: EPriceType = EPriceType.PRTP_FIX,
        operation_type: EOperationType = EOperationType.OPT_OPEN_LONG,
        remark: str = "",
    ):
        orderInfo = COrdinaryOrder()
        orderInfo.m_strAccountID = account_id  # 资金账号
        orderInfo.m_dPrice = price  # 报单价格
        orderInfo.m_dSuperPriceRate = super_price_rate  # 单笔超价百分比，选填字段。
        orderInfo.m_nVolume = volume  # 报单委托量
        orderInfo.m_strMarket = market  # 报单市场
        orderInfo.m_strInstrument = code  # 报单合约代码
        orderInfo.m_ePriceType = price_type  # 报单价格类型
        orderInfo.m_eOperationType = operation_type  # 报单委托类型
        orderInfo.m_strRemark = ""  # 备注
        self.m_nRequestId += 1
        self.m_api.order(orderInfo, self.m_nRequestId, account_key)  # 下单
        print(
            "[futureOrdinaryOrder]期货普通单, accountId:",
...
if __name__ == "__main__":
    serverAddr = "183.57.40.218:65300"
    username = "唐斌"
    password = "yml666888"
    apiclient = xtTraderApiClient(serverAddr, username, password)
    apiclient.init()

    # 等待主推到来（等待accountKey字典里有你的账号，并且状态为OK）
    accountID = "1014609"
    # 循环等待，直到主推到达
    while (
        accountID not in apiclient.m_dictAccountID2Key
        or apiclient.m_dictAccountKeyStatus.get(
            apiclient.m_dictAccountID2Key[accountID]
        )
        != EBrokerLoginStatus.BROKER_LOGIN_STATUS_OK
    ):
        time.sleep(0.1)
    # 拿到最新可用key
    accountKey = apiclient.m_dictAccountID2Key[accountID]

    # !!! 用这个accountKey下单就绝不会报“未找到账号或者工作流为空” !!!
    apiclient.future_order(
        accountID,
        accountKey,
        22.5,
        0.2,
