...
def win_rate(data_dict: dict, hold: dict, prehold: dict, cost: dict, tradeprice: dict):
    result = pd.Series()
    for code, df in data_dict.items():
        if hold[code] > 0:
            todayclose = df['close'].iloc[-1]
            if prehold[code] > 0:
                yesterdayclose = df['close'].iloc[-2]
                result[code] = (todayclose > yesterdayclose)
            else:
                result[code] = (todayclose > cost[code])
        else:
            yesterdayclose = df['close'].iloc[-2]
            result[code] = (tradeprice[code] > yesterdayclose)
    count = result.sum()
    rate = count / len(data_dict) if len(data_dict) != 0 else 0
    return rate
...
def init(C):
    try:
        ...
        print(C.acc_s)
        g.miniQuote_path = r'C:\迅投投资交易系统交易终端 优美利私募模拟\miniQuote\userdata_mini' # 需要改路径
        C.xt_trader = None
