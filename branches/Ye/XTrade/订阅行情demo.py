...
class CallBack(XtTraderApiCallback):
    ...
    def testUserLoginSync(self, username, password, machineInfo, appid, authcode):
        ...
        error = self.m_client.userLoginSync(
            username, password, machineInfo, appid, authcode
        )
        if error.isSuccess():
            self.m_nLogined = 1
            print("userLoginSync result:", error.isSuccess())
            self.doRequest()
        else:
    ...
    def doRequest(self):
        ...
        if self.m_nLogined:
            ...
            # 同步请求测试demo
            # 行情订阅示例
            # self.testsubscribQuote("CFFEX", "IC2509")
            # 批量订阅行情示例
            self.testBatchSubscribQuote()
            # 主动请求对应合约行情
            # self.reqPriceDataSync("SHFE", "ni2509")
    ...
    def testBatchSubscribQuote(self):
        ...
        for market, stockcode, platformId in self.m_codeList:
            print(
                "添加订阅参数 市场：{}， 合约：{}， 平台：{}".format(
                    market, stockcode, platformId
                )
            )
            subscribdata = CSubscribData()  # 初始化数据，行情订阅结构体
            subscribdata.m_strExchangeID = market
    ...
    def onSubscribQuote(self, request_id, data, error):
        print("[onSubscribQuote] m_strInstrumentID:", data.m_strInstrumentID)
