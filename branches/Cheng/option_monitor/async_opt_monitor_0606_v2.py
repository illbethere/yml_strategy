...
class TradingMonitorGUI:
    def __init__(self, root):
        ...
        self.option_codes = ['sn2507P245000.SF', 'ag2508P8100.SF', 'ag2508C9100.SF', 'ag2508C9300.SF', 'si2509-P-7100.GF', 'si2509-P-7000.GF', 'si2509-P-7200.GF', 'rb2510P2850.SF','rb2510P2900.SF','sn2507P240000.SF','ao2508C3100.SF', 'ao2509C3100.SF','ao2509P2800.SF','ag2508P8300.SF','ag2508P7800.SF','ag2508P8000.SF', 'cu2507P73000.SF', 'SH509C2560.ZF', 'SR509C6200.ZF', 'a2509-C-4450.DF', 'ps2508-C-36000.GF',  'ni2507C128000.SF','lc2509-C-64000.GF'] # 已成交合约
        self.main_contract = []
        self.dict_history = {}
        ...
        self.option_positions = {
            ...
            'ag2508C9100.SF': {'cost_price': 75, 'position': 1, 'multiplier': 15},
            'ag2508C9300.SF': {'cost_price': 52, 'position': 1, 'multiplier': 15},
            'ps2508-C-36000.GF': {'cost_price': 206, 'position': 2, 'multiplier': 3},
            'ao2509C3100.SF': {'cost_price': 54.36, 'position': 11, 'multiplier': 20},
            'ao2509P2800.SF': {'cost_price': 65, 'position': 1, 'multiplier': 20},
            'lc2509-C-64000.GF': {'cost_price': 650, 'position': 1, 'multiplier': 1},
            'ao2508C3100.SF': {'cost_price': 26.5, 'position': 1, 'multiplier': 20},
            'rb2510P2850.SF': {'cost_price': 40, 'position': 3, 'multiplier': 10},
            'rb2510P2900.SF': {'cost_price': 54.5, 'position': 1, 'multiplier': 10},
            'si2509-P-7100.GF': {'cost_price': 61.2, 'position': 6, 'multiplier': 5},
            'si2509-P-7000.GF': {'cost_price': 47, 'position': 5, 'multiplier': 5},
            'si2509-P-7200.GF': {'cost_price': 71.4, 'position': 8, 'multiplier': 5},
            'ag2508P8100.SF': {'cost_price': 33.5, 'position': 2, 'multiplier': 15},
            'sn2508P245000.SF': {'cost_price': 620, 'position': 1, 'multiplier': 1},


        } # 此处添加期权代码，成交价格 -> cost-price，手数 -> position以及每手交易单位 -> multiplier,添加完后需要再self.option_codes中添加对应的期权代码


        self.alarm_price = {'ag2508.SF': 8478.261 * self.alarm_threshold_put,
                            'cu2507.SF': 79060 * self.alarm_threshold_put} # 合约成交时标的的价格 * 预警系数
    ...
    def setup_ui(self):
        ...
        self.option_update_label = ttk.Label(option_frame, text="上次更新: ", font=("Arial", 9))
        self.option_update_label.pack(anchor=tk.W)

        self.option_tree = ttk.Treeview(option_frame, columns=('current_price', 'gain_loss', 'gain_percent', 'position', 'total_gain'), show='tree headings', height=15)
        ...
        for col in ('current_price', 'gain_loss', 'gain_percent', 'position', 'total_gain'):
            self.option_tree.column(col, width=100)

        option_scrollbar = ttk.Scrollbar(option_frame, orient=tk.VERTICAL, command=self.option_tree.yview)
        ...
        # 左侧 - 1分钟数据
        left_frame = ttk.LabelFrame(content_frame, text="1分钟数据 (所有期货) 红色 -> sell call | 绿色 -> sell put | 黄色 -> 现价接近最高最低价1%范围内", padding="5")
    ...
    def setup_data(self):
        ...
        try:
            ...
            dict_data = self.get_market_data(self.contract_deal, '1d', '20250407', today, 21)
            dict_data_all = self.get_market_data(self.main_contract, '1d', '20250407', today, 21)


            
            for contract in self.contract_deal:
                if contract in dict_data and not dict_data[contract].empty:
                    dict_data_20 = dict_data[contract].iloc[:-1]
                    # print(dict_data)
                    
                    # print(dict_data_all)
                    close_min = dict_data_20['close'].min()
                    close_max = dict_data_20['close'].max()
            ...
            for contract in self.main_contract:
                dict_data_all_20 = dict_data_all[contract].iloc[:-1]
                close_min = dict_data_all_20['close'].min()
                close_max = dict_data_all_20['close'].max()
            ...
            # 初始化tick数据的计数
            tick_stats = self.init_signal_stats(self.contract_deal, self.dict_history, morning_start, today)
            ...
            # 初始化1分钟数据的计数
            min1_stats = self.init_signal_stats(self.main_contract, self.dict_history_all, morning_start, today)
            self.df_results_all['pos'] = [min1_stats[c]['pos'] for c in self.main_contract]
            self.df_results_all['neg'] = [min1_stats[c]['neg'] for c in self.main_contract]
    ...
    def update_treeview(self, tree, df, update_time_label, data_type):
        ...
        for contract, row in df_sorted.iterrows():
            ...
            if row['signal'] == 1:
                ...
                if data_type == "1分钟" and not pd.isna(row['price']) and not pd.isna(row['min']) and not pd.isna(row['max']):
        ...
        try:
            ...
            for option_code in self.option_codes:
                if option_code in option_data and option_code in self.option_positions:
                    ...
                    item = self.option_tree.insert('', tk.END, text=option_code, values=values)
                    ...
                    self.option_tree.tag_configure('total', background='lightyellow', font=('Arial', 10, 'bold'))                    
        ...
        try:
            ...
            if need_1min_update:
                ...
                for contract in self.main_contract:
                    ...
                    # current_signal = self.df_results_all.at[contract, 'signal']

                    # if pd.isna(count_pos) or isinstance(count_pos, pd.Series):
                    #     count_pos = 0
                    # else:
                    #     count_pos = int(count_pos)

                    # if pd.isna(count_neg) or isinstance(count_neg, pd.Series):
                    #     count_neg = 0
                    # else:
                    #     count_neg = int(count_neg)

                    # if pd.isna(current_signal) or isinstance(current_signal, pd.Series):
                    #     current_signal = 0
                    # else:
                    ...
                    try:
                        count_pos_raw = self.df_results_all.at[contract, 'pos']
                        if isinstance(count_pos_raw, pd.Series):
                            count_pos = 0
                        elif pd.isna(count_pos_raw):
                            count_pos = 0
                        else:
                    ...
                    try:
                        count_neg_raw = self.df_results_all.at[contract, 'neg']
                        if isinstance(count_neg_raw, pd.Series):
                            count_neg = 0
                        elif pd.isna(count_neg_raw):
                            count_neg = 0
                        else:
                            count_neg = int(count_neg_raw)
                    except:
                        count_neg = 0

                    try:
                        current_signal_raw = self.df_results_all.at[contract, 'signal']
                        if isinstance(current_signal_raw, pd.Series):
                            current_signal = 0
                        elif pd.isna(current_signal_raw):
                            current_signal = 0
                        else:
                            current_signal = int(current_signal_raw)
                    except:
                        current_signal = 0

                    if close_now > close_max and (current_time.time() >= morning_start and current_time.time() <= morning_end or \
                        current_time.time() >= afternoon_start and current_time.time() <= afternoon_end or \
                        current_time.time() >= night_start and current_time.time() <= night_end):
                        count_pos += 1
                        self.df_results_all.at[contract, 'pos'] = count_pos
                    elif close_now < close_min and (current_time.time() >= morning_start and current_time.time() <= morning_end or \
                        current_time.time() >= afternoon_start and current_time.time() <= afternoon_end or \
                        current_time.time() >= night_start and current_time.time() <= night_end):
                        count_neg += 1
                        self.df_results_all.at[contract, 'neg'] = count_neg

                    if count_pos >= 1 and current_signal != 1:
                        self.df_results_all.at[contract, 'strike'] = close_now * (1 - self.n)
                    elif count_neg >= 1 and current_signal != -1:
                        self.df_results_all.at[contract, 'strike'] = close_now * (1 + self.n)
                    ...
                    if count_pos >= 10 and current_signal != 1:
                    ...
                    elif count_neg >= 10 and current_signal != -1:
                        self.df_results_all.at[contract, 'signal'] = -1
            ...
            for contract in self.contract_deal:
                ...
                # current_signal = self.df_results.at[contract, 'signal']
                ...
                try:
                    count_pos_raw = self.df_results.at[contract, 'pos']
                    if isinstance(count_pos_raw, pd.Series):
                        count_pos = 0
                    elif pd.isna(count_pos_raw):
                        count_pos = 0
                ...
                try:
                    count_neg_raw = self.df_results.at[contract, 'neg']
                    if isinstance(count_neg_raw, pd.Series):
                        count_neg = 0
                    elif pd.isna(count_neg_raw):
                        count_neg = 0
                ...
                try:
                    current_signal_raw = self.df_results.at[contract, 'signal']
                    if isinstance(current_signal_raw, pd.Series):
                        current_signal = 0
                    elif pd.isna(current_signal_raw):
                        current_signal = 0
                ...
                # if pd.isna(count_pos):
                #     count_pos = 0
                # if pd.isna(count_neg):
                #     count_neg = 0
                # if pd.isna(current_signal):
                #     current_signal = 0

                if contract in self.alarm_price and close_now < self.alarm_price[contract]:
