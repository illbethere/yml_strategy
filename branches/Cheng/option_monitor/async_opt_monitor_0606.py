...
class TradingMonitorGUI:
    def __init__(self, root):
        ...
        self.option_codes = ['ag2508P7800.SF', 'cu2507P73000.SF', 'SH509C2560.ZF', 'SR509C6200.ZF', 'a2509-C-4450.DF'] # 已成交合约
        self.main_contract = []
        self.dict_history = {}
        ...
        self.option_positions = {
            ...
            'SH509C2560.ZF': {'cost_price': 31.25, 'position': 2, 'multiplier': 20}, 
            'SR509C6200.ZF': {'cost_price': 9, 'position': 8, 'multiplier': 10},
            'a2509-C-4450.DF': {'cost_price': 16.5, 'position': 1, 'multiplier': 10},
    ...
    def setup_ui(self):
        ...
        self.option_tree = ttk.Treeview(option_frame, columns=('current_price', 'gain_loss', 'gain_percent', 'position', 'total_gain'), show='tree headings', height=4)
    ...
    def update_treeview(self, tree, df, update_time_label, data_type):
        ...
        for contract, row in df_sorted.iterrows():
            ...
            if row['signal'] == 1:
                ...
                if data_type == "1分钟" and not pd.isna(row['price']) and not pd.isna(row['min']) and not pd.isna(row['max']) and row['signal'] != 1 and row['signal'] != -1:
