...
namespace xti
{
    ...
    struct XT_API_EXPORT CAccountDetail
    {
        ...
        char        m_strAccountID[64];         ///< 资金账号
        int         m_nAccountType;             ///< 账号类型
        char        m_strStatus[32];            ///< 账号状态
        char        m_strTradingDate[16];       ///< 交易日

        double      m_dFrozenMargin;            ///< 冻结保证金 股票不需要
        ...
        double      m_dAssetBalance;            ///< 动态权益含期权
        double      m_dRiskDegree;              ///< 风险度含期权
        double      m_dInitCloseMoney;          ///< 初始平仓盈亏

    };

    ///< 信用账号详细信息 后面不再维护，改用 CCreditDetail
    ...
    struct XT_API_EXPORT CNetValue
    {
        ...
        double      m_dShare;               ///< 产品份额
        double      m_dTotalIncome;         ///< 当日产品盈亏
        double      m_dAccumulateIncome;    ///< 产品累计盈亏
        double      m_dCloseTotalIncome;    ///< 产品收盘盈亏
        double      m_dPrevTotalNetValue;   ///< 前日产品净值
        double      m_dPrevNetValue;        ///< 前日单位净值
        double      m_dAccumulateNetAssetValue; //累计单位净值
        double      m_dAccumulateNetValue; //产品累计净值
    };
    ...
    */
    struct XT_API_EXPORT COrderInfo
    {
        ...
        int                             m_endTime;           ///< 结束时间
        EOrderCommandStatus             m_eStatus;           ///< 指令状态
        ...
        double                          m_dPrice;                   ///< 委托基准价  
        int                             m_nVolume;                  ///< 下单总量  
        int                             m_nValidTimeStart;          ///< 有效开始时间  时间戳格式
        int                             m_nValidTimeEnd;            ///< 有效结束时间  时间戳格式
        ...
        char                            m_strOtherParam[8192];      ///< 条件单其他参数
        int                             m_nCmdSource;               ///< 指令来源，比如普通客户端类型、API等
    };
    ...
    struct XT_API_EXPORT COrderDetail
    {
        ...
        int                         m_nOrderID;             ///< 指令ID
        EBrokerPriceType            m_nOrderPriceType;      ///< 委托明细价格类型，例如市价单 限价单
        ...
        EEntrustSubmitStatus        m_eOrderSubmitStatus;   ///< 期货提交状态，股票里面不需要报单状态
    ...
    struct XT_API_EXPORT CDealDetail
    {
        ...
        char                m_strAccountID[64];
        int                 m_nAccountType;             ///< 账号类型
        char                m_strExchangeID[16];        ///< 交易所代码
        char                m_strProductID[32];         ///< 合约品种
        char                m_strInstrumentID[32];      ///< 合约代码
        char                m_strTradeID[64];           ///< 成交编号
        char                m_strOrderSysID[32];        ///< 委托号
        double              m_dAveragePrice;            ///< 成交均价
        ...
        char                m_strProductName[64];       ///< 迅投产品名称
        char                m_strCombID[32];            ///< 组合持仓编号，用于解除组合策略
        double              m_dOccupedMargin;           ///< 占用保证金
        double              m_dReferenceRate;           ///< 汇率
        char                m_strEntryDate[32];         ///< 投资组合-录入日期
        char                m_strExpireDate[32];        ///< 投资组合-解禁日期
        PortfolioDealType   m_nPortfolioDealType;       ///< 投资组合成交类型
    };
    ...
    struct XT_API_EXPORT CPositionDetail
    {
        ...
        char        m_strTradeID[64];       ///< 最初开仓位的成交编号
    ...
    struct XT_API_EXPORT CPositionStatics
    {
        ...
        char        m_strAccountKey[64];    ///< 账号key
        char        m_strSecuAccount[32];   ///< 股东号
        ESideFlag   m_eSideFlag;            ///< 期权合约持仓类型
        double      m_dReferenceRate;       ///< 汇率
        char        m_strExpireDate[32];    ///< 到期日/投资组合解禁日
        PortfolioPositionType      m_nPortfolioPositionType;   ///< 投资组合持仓类型
        double      m_dSingleCost;          ///< 单股成本
    };

    ///< 期货持仓统计
    struct XT_API_EXPORT CFuturePositionStatics
    {
        ...
        char        m_strAccountID[64];     ///< 资金账号
        char        m_strExchangeID[16];    ///< 交易所
        char        m_strExchangeName[32];  ///< 市场名称
        char        m_strProductID[32];     ///< 品种代码
        char        m_strInstrumentID[32];  ///< 合约代码
        char        m_strInstrumentName[64];///< 合约名称
        EEntrustBS          m_nDirection;   ///< 期货多空，该字段与m_eOffsetFlag一起判断期货的报单类型
        EHedgeFlagType      m_nHedgeFlag;   ///< 投机 套利 套保
        int         m_nPosition;            ///< 总持仓 
        int         m_nYestodayPosition;    ///< 昨仓 
        int         m_nTodayPosition;       ///< 今仓 
        int         m_nCanCloseVol;         ///< 可平量 
        double      m_dPositionCost;        ///< 持仓成本 detail的汇总
        double      m_dAvgPrice;            ///< 持仓均价
        double      m_dPositionProfit;      ///< 持仓盈亏 detail的汇总
        double      m_dFloatProfit;         ///< 浮动盈亏 detail的汇总
        double      m_dOpenPrice;           ///< 开仓均价 
        double      m_dUsedMargin;          ///< 已使用保证金
        double      m_dUsedCommission;      ///< 已使用的手续费
        double      m_dFrozenMargin;        ///< 冻结保证金
        double      m_dFrozenCommission;    ///< 冻结手续费
        double      m_dInstrumentValue;     ///< 合约价值
        int         m_nOpenTimes;           ///< 开仓次数
        int         m_nOpenVolume;          ///< 总开仓量 
        int         m_nCancelTimes;         ///< 撤单次数
        double      m_dLastPrice;           ///< 最新价
        double      m_dRiseRatio;           ///< 当日涨幅
        char        m_strProductName[256];  ///< 产品名称
        double      m_dRoyalty;             ///< 权利金市值
        char        m_strExpireDate[32];    ///< 到期日
        double      m_dAssestWeight;        ///< 资产占比
        double      m_dIncreaseBySettlement;///< 当日涨幅（结）
        double      m_dMarginRatio;         ///< 保证金占比
        double      m_dFloatProfitDivideByUsedMargin; ///< 浮盈比例（保证金）
        double      m_dFloatProfitDivideByBalance;    ///< 浮盈比例（动态权益）
        double      m_dTodayProfitLoss;     ///< 当日盈亏（结）
        int         m_nYestodayInitPosition;///< 昨日持仓 
        double      m_dFrozenRoyalty;       ///< 冻结权利金 
        double      m_dTodayCloseProfitLoss; ///< 当日盈亏（收） 
        double      m_dCloseProfit;         ///< 平仓盈亏
        char        m_strFtProductName[32]; ///< 品种名称
        double      m_dOpenCost;            ///< 开仓成本
        double      m_dEstimateSettleProfitLoss;    ///< 当日盈亏（预结）
        double      m_dEstimateSettleMargin;///< 保证金（预结）
        double      m_dSettlementPrice;     ///< 结算价
    ...
    struct XT_API_EXPORT COrdinaryOrder
    {
        COrdinaryOrder();
        ~COrdinaryOrder();
        char                m_strAccountID[64];            ///< 资金账户ID，如果为子账户，则为子账户ID
        double              m_dPrice;                      ///< 委托价格，仅在报价方式为PRTP_FIX(指定价)时有效;
        ...
        OrderPriceType      m_eLimitOrderPriceType;        ///< 限价单价格限制类型 GFD FAK FOK
    };
    ...
    struct XT_API_EXPORT CAlgorithmOrder
    {
        ...
        int                 m_nValidTimeStart;          ///< 有效开始时间 来自股票，待定 时间戳格式
        int                 m_nValidTimeEnd;            ///< 有效结束时间 来自股票，待定 时间戳格式
        ...
        EStopTradeForOwnHiLow   m_nStopTradeForOwnHiLow;    ///< 涨跌停控制 拆单算法只能选择涨停不卖跌停不买 STOPTRADE_NO_BUY_SELL 
        OrderPriceType      m_eLimitOrderPriceType;        ///< 限价单价格限制类型 GFD FAK FOK
    };
    ...
    struct XT_API_EXPORT CIntelligentAlgorithmOrder
    {
        ...
        ~CIntelligentAlgorithmOrder();
        char                    m_strAccountID[64];             ///< 账号，如果是多账号下单，该字段无意义
        char                    m_strMarket[32];                ///< 市场
        char                    m_strInstrument[32];            ///< 合约
    ...
    ///< 华创算法单下单参数
    struct XT_API_EXPORT CHuaChuangAlgorithmOrder
    {
        ...
        char                    m_strMarket[32];            ///< 市场
        char                    m_strInstrument[32];        ///< 合约
        char                    m_strOrderType[32];         ///< 算法名称，TWAP,VWAP,VWAPPLUS,STRICTTWAP,PriceInLine,POV,PEGGING,SMARTTWAP,SMARTVWAP
        double                  m_dPrice;                   ///< 基准价
        int                     m_nVolume;                  ///< 下单总量
        EOperationType          m_eOperationType;           ///< 下单操作：买入卖出
        EOrderStrategyType      m_eOrderStrategyType;       ///< 算法下单方式
        char                    m_strTimeStart[128];        ///< 开始时间，格式yyyyMMdd-HH:mm:ss.fff  默认当日开盘时间
        char                    m_strTimeEnd[128];          ///< 结束时间，格式yyyyMMdd-HH:mm:ss.fff  默认当日收盘时间
        ...
        double                  m_dMaxPartRate;             ///< 量比比例, 用户设定, 当MaxPartRate==100%, 表示没有限制
        double                  m_dMinAmountPerOrder;       ///< 委托最小金额
        char                    m_strOtherParam[1024];      ///< 华创算法额外扩展参数
    };

    ///< 华创算法组合单下单参数
    struct XT_API_EXPORT CHuaChuangAlgoGroupOrder
    {
        ...
        double                        m_dPrice[1000];             ///< 每只股票的基准价，限价必填，市价不填
    ...
    struct XT_API_EXPORT COrdinaryGroupOrder
    {
        ...
        COrdinaryGroupOrder();
        ~COrdinaryGroupOrder();
        char                          m_strAccountID[64];        ///< 资金账户ID，如果为子账户，则为子账户ID ，如果是多账号下单，该字段无意义
        double                        m_dSuperPriceRate;         ///< 单笔超价百分比
        EPriceType                    m_ePriceType;              ///< 报价方式： 指定价，最新价 对手价……
        EHedgeFlagType                m_eHedgeFlag;              ///< 套利标志
        EXtOverFreqOrderMode          m_eOverFreqOrderMode;      ///< 委托频率过快时的处理方式
    ...
    struct XT_API_EXPORT CProductData
    {
        ...
        double      m_dGGTStockValue;                   ///<  港股市值 （人民币）
        double      m_dShare;                           ///<  产品份额
        double      m_dTotalIncome;                     ///<  当日产品盈亏
        double      m_dAccumulateIncome;                ///<  产品累计盈亏
        double      m_dCloseTotalIncome;                ///<  产品收盘盈亏
    ...
    struct XT_API_EXPORT CStockOptionCombPositionDetail
    {
        ...
        char        m_strCombCodeName[64];      ///< 组合策略名称
        int         m_nVolume;                  ///< 总持仓量
    ...
    struct XT_API_EXPORT CCommissionRateDetail
    {
        ...
        char            m_strExchangeID[32];            ///< 市场代码
        char            m_strInstrumentID[32];          ///< 合约代码
        double          m_dOpenRatioByMoney;            ///< 开仓手续费率(按金额)
        double          m_dOpenRatioByVolume;           ///< 开仓手续费率(按手)
        double          m_dCloseRatioByMoney;           ///< 平仓手续费率(按金额)
        double          m_dCloseRatioByVolume;          ///< 平仓手续费率(按手)
        double          m_dCloseTodayRatioByMoney;      ///< 平今手续费率(按金额)
        double          m_dCloseTodayRatioByVolume;     ///< 平今手续费率(按手)
    ...
    struct XT_API_EXPORT CMarginRateDetail
    {
        ...
        double          m_dLongMarginRatioByMoney;      ///< 按金额多头保证金率
        double          m_dLongMarginRatioByVolume;     ///< 按数量多头保证金费
        double          m_dShortMarginRatioByMoney;     ///< 按金额空头保证金率
        double          m_dShortMarginRatioByVolume;    ///< 按数量空头保证金费
    ...
    ///< ETF申赎清单
    struct XT_API_EXPORT CETFPCFDetail
    {
        CETFPCFDetail();
        ~CETFPCFDetail();
        int                           m_nType;                  ///< 基金类型
        char                          m_strEtfExchID[16];       ///< ETF市场
        char                          m_strEtfCode[32];         ///< ETF代码
        char                          m_strPrCode[32];          ///< 基金申赎代码
        char                          m_strName[256];           ///< 基金名称
        double                        m_dCashBalance;           ///< 现金差额
        double                        m_dMaxCashRatio;          ///< 现金替代比例上限
        int                           m_nReportUnit;            ///< 最小申购,赎回单位 单位:份
        double                        m_dNavPerCU;              ///< 最小申购,赎回单位净值 单位:元
        double                        m_dNav;                   ///< 基金份额净值 单位:元
        double                        m_dEcc;                   ///< 预估现金差额 单位:元
        int                           m_nNeedPublish;           ///< 是否需要公布IOPV 1,是,0,否
        int                           m_nEnableCreation;        ///< 是否允许申购 1,是,0,否
        int                           m_nEnableRedemption;      ///< 是否允许赎回 1,是,0,否
        int                           m_nCreationLimit;         ///< 申购上限 单位:份,0 表示不限制
        int                           m_nRedemptionLimit;       ///< 赎回上限 单位:份,0 表示不限制
        char                          m_strTradingDay[32];      ///< ETF交易日期 格式 YYYYMMDD
        char                          m_strPreTradingDay[32];   ///< ETF前交易日期 格式 YYYYMMDD

    };

    ///< ETF成分股信息
    struct XT_API_EXPORT CETFComponentStockInfo
    {
        CETFComponentStockInfo();
        ~CETFComponentStockInfo();
        char                          m_strExchangeID[16];      ///< ETF市场
        char                          m_strEtfCode[32];         ///< ETF代码
        char                          m_strEtfName[256];        ///< ETF基金名称
        char                          m_strComponentCode[32];   ///< 成份股代码
        char                          m_strComponentName[256];  ///< 成份股名称
        int                           m_nComponentVolume;       ///< 成份股数量
        char                          m_strComponentExchID[16]; ///< 成份股市场
        EReplaceFlag                  m_eReplaceFlag;           ///< 替代标志
        double                        m_dReplaceRatio;          ///< 溢价比率
        double                        m_dReplaceBalance;        ///< 替代金额
        double                        m_dDiscountRatio;         ///< 赎回折价比率
        double                        m_dRedeemReplaceBalance;  ///< 赎回替代金额

    };

    ///< 用户配置信息
    struct XT_API_EXPORT CUserConfig
    {
        CUserConfig();
        ~CUserConfig();
        int                           m_nId;       ///< 
        char                          m_strUser[32];      ///< 用户
        char                          m_strUpdateTime[16];      ///< 更新时间
        char                          m_strContent[64];      ///< 用户
        char                          m_strDescription[512];      ///< 描述
        int                           m_nMetaId;       ///< 数据ID 
        char                          m_bsonContent[1024];      ///<  内容串
        int                           m_nPermission;       ///< 私人还是全局
    };

    ///< 资金流水信息
    struct XT_API_EXPORT CFundFlow
    {
        CFundFlow();
        ~CFundFlow();
        char                          m_strAccountID[64];       ///< 资金账户ID，股票，两融，深港通，沪港通，个股期权
        int                           m_nBizDate;               ///< 发生日期，股票，两融，深港通，沪港通，个股期权
        int                           m_nBizTime;               ///< 发生时间，股票，两融，深港通，沪港通，个股期权
        char                          m_strSerialNo[32];        ///< 流水序号，股票，两融，深港通，沪港通，个股期权
        char                          m_strBizName[32];         ///< 业务名称，股票，两融，深港通，沪港通，个股期权
        double                        m_dBizBalance;            ///< 发生金额，股票，两融，深港通，沪港通，个股期权
        double                        m_dBalance;               ///< 剩余金额，股票，两融，深港通，沪港通，个股期权
        EMoneyType                    m_eMoneyType;             ///< 币种，股票，两融，深港通，沪港通，个股期权
        char                          m_strMarket[32];          ///< 市场，股票，两融，深港通，沪港通，个股期权
        char                          m_strInstrument[32];      ///< 合约，股票，两融，深港通，沪港通，个股期权
        char                          m_strInstrumentName[64];  ///< 合约名称，股票，两融
        char                          m_strBizType[32];         ///< 业务名称，股票，两融
        double                        m_dBizPrice;              ///< 成交价格，股票，两融
        double                        m_dBizPriceBalance;       ///< 成交金额，股票，两融
        double                        m_dCommission;            ///< 手续费，股票，两融
        double                        m_dStampTax;              ///< 印花税，股票，两融
        double                        m_dTransFee;              ///< 过户费，股票，两融
        double                        m_dOtherFee;              ///< 其他费用，股票，两融
        double                        m_dChangeAmount;          ///< 发生数量/金额，股票，两融
        double                        m_dPostAmount;            ///< 剩余数量/金额，股票，两融
        double                        m_dBizOrderPrice;         ///< 委托价格，股票，两融
        double                        m_dBizOrderAmount;        ///< 委托数量，股票，两融
        char                          m_strStockAccount[32];    ///< 股东账号，股票，两融 
        char                          m_strBizNo[32];           ///< 成交序列号，股票，两融 
        char                          m_strEntrustNo[32];       ///< 委托号，股票，两融 
        char                          m_strEntrustBS[32];       ///< 买卖方向，股票，两融 
        char                          m_strNotes[32];           ///< 备注，股票，两融 
    };
}
