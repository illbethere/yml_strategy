...
namespace demo3
{
    ...
    void Callback::testFutureOrdinaryOrder(const string& accountId)
    {
        // 参数中所有char[]数组默认值都为空。
        COrdinaryOrder orderInfo;
        ...
        // 报单合约代码，必填字段。
        strcpy(orderInfo.m_strInstrument, "IF2109");
    ...
    void Callback::onUserLogin(const char* userName, const char* password, int nRequestId, const XtError& error)
    {
        ...
        if (error.isSuccess())
        {
            ...
            subInfo1.m_nPlatformID = 10000;
            strcpy(subInfo1.m_strExchangeID, MARKET_SHANGHAI);
            ...
            subInfo2.m_nPlatformID = 20001;
            strcpy(subInfo2.m_strExchangeID, MARKET_INDEX_FUTURE);
            strcpy(subInfo2.m_strInstrumentID, "IF2109");
    ...
    void Callback::onSubscribQuote(int nRequestId, const CSubscribData* data, const XtError& error)
    {
        if (error.isSuccess())
        {
            cout << "onSubscribQuote success " <<endl;
        }
        else
        {
            cout << "onSubscribQuote failed errmsg:" << error.errorMsg() << endl;
        }
    ...
    {
        cout<<"[onRtnPriceData] receive quote: "<<data.m_strExchangeID<<"-"<<data.m_strInstrumentID << ", price: " << data.m_dLastPrice << endl;
        if (strcmp(data.m_strExchangeID, MARKET_SHANGHAI))
        {
            //testStockOrdinaryOrder(m_strStockAccount);
        }
        else if (strcmp(data.m_strExchangeID, MARKET_INDEX_FUTURE))
        {
            //testFutureOrdinaryOrder(m_strFutureAccount);
        }
    }
