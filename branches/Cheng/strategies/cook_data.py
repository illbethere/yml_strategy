...
def get_trading_date(dt,calendar:list):
    ...
    date_list = xtdata.get_trading_dates('SF', start_time='', end_time=today, count=100)
        ...
        if hour >= 21:
            try:
                ...
                if idx < len(calendar):
                    ...
                    if trading_date not in date_list:
                        trading_date = calendar[idx + 1]

                    # trading_date_next = calendar[idx + 1]
                    # if trading_date_next.date() - trading_date.date() > 1:
                    #     trading_date = trading_date_next
                    # if trading_date.weekday() == 5:  
                    #     trading_date = calendar[idx + 2]
                else:
                    print("日期超出交易日历范围")
            ...
            if weekday == 5:
                try:
                    ...
                    if idx < len(calendar):
                        ...
                        trading_date = current_date + timedelta(days=2)
                except:
                    import traceback
                    traceback.print_exc()
                # trading_date = trading_date.index(dt.date()) + 1
            elif weekday == 6:
                try:
                    idx = calendar.index(dt.date()) + 1
                    if idx < len(calendar):
                        ...
                        trading_date = current_date + timedelta(days=1)
                except:
                    import traceback
                    traceback.print_exc()
                # trading_date = trading_date.index(dt.date()) + 1
