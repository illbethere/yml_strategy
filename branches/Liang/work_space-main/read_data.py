...
def get_singel_market_data(
    code: str, period: str, start_time: str, end_time: str, count: int
) -> pd.DataFrame:
    """获取单个市场数据"""
    market_data = xtdata.get_market_data_ex(
        [],
...
def get_main_contract_code(
    continuous_contract_code: str, start_time: str, end_time: str
) -> str:
    ...
    dt_str = dt.strftime('%Y%m%d')
    print(dt_str)
    main_df = get_singel_market_data(
    ...
    main_contract = main_df[main_df["time"] < start_time].iloc[
        -1, 1
    ]  # 获取开始时间之前的最后一个主力合约代码
    main_contract = str(main_contract) + "." + exchange_code  # 添加交易所代码后缀
