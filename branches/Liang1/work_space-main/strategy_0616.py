...
class StrategyManager:
    ...
    # xtdata.get_option_detail_data()函数中，对于TA2509.ZF和TA509.ZF，会只接受TA509.ZF
    def get_shorter_options(self, option_dict):
        # 按值分组
        value_groups = {}
        ...
        # 对每个值组，选择最短的键,例如TA2509.ZF和TA509.ZF都对应行权价2509，取TA509.ZF
        filtered_dict = {}
    ...
    # xtdata.get_option_list()函数中，对于TA509.ZF和TA2509.ZF，会接受TA2509.ZF
    def fill_longer_future(self, code):
        """
        将短格式的期货代码转换为长格式
        例如: TA509.ZF -> TA2509.ZF, SH511.ZF -> SH2511.ZF
        """
        ...
        letters = base_code[:i]  # 字母部分，如 'TA', 'SH', 'SR'
        numbers = base_code[i:]  # 数字部分，如 '509', '511', '505'
    ...
    def strategy(self, continuous_contract_code: str, start_date: str, end_date: str, max_signals: int = 30, lookback_days: int = 20, k: float = 0.08, close_n: int = 10, alarm_ratio: float = 0.02) -> pd.DataFrame:
        ...
        # 获取当前期货对应的期权列表
        code = self.fill_longer_future(code)  # 将ZF的短格式的期货代码转换为长格式
        ...
        print(option_list_PUT)
        print(option_list_CALL)
        ...
        for i in range(len(future_df)):
            ...
            if future_df.at[i, 'signal_type'] == 1 and self.order_manager.open_orders == {}:
                ...
                if option_info_dict:
                    ...
                    print(f"选择的期权: {selected_option}, 类型: PUT, 行权价: {selected_strike}, OpenDate: {current_date}, ExpireDate: {expire_date}")

                    # 获取当前期权的最新价格
                    if i < len(option_df):
                        current_option_price = option_df.iloc[i]['close']
                    else:
                        print(f"索引 {i} 超出期权数据范围，无法获取当前价格")
                        continue

                    if day_to_open < (expire_to_open - close_n):
                        # 如果距离行权日还有足够的时间，则可以开仓
                        # Sell Put
            ...
            if future_df.at[i, 'signal_type'] == -1 and self.order_manager.open_orders == {}:
                ...
                if option_info_dict:
                    ...
                    print(f"选择的期权: {selected_option}, 类型: CALL, 行权价: {selected_strike}, OpenDate: {current_date}, ExpireDate: {expire_date}")
                ...
                for order_id, order_info in list(self.order_manager.open_orders.items()):
                    ...
                    if self.parse_option_code(order_info['code'])['option_type'] == 'call':
                        ...
                        if current_close >= alarm_price or day_to_open >= (expire_to_open - close_n) or current_time == end_trading_time:
                            ...
                            print(f"预警价格: {alarm_price:.2f} (CALL), open_future_price: {open_future_price:.2f}, day_to_open: {day_to_open}, expire_to_open: {expire_to_open}, dynamic_alarm_ratio: {dynamic_alarm_ratio:.2f}")
