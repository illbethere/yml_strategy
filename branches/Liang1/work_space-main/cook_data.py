...
def get_trading_date(dt,calendar:list) -> str:
    ...
    if hour >= 21 :
        # 如果是21:00~23:59之间的时间，交易日是当天的下一个交易日
        idx = calendar.index(today) + 1
        if idx < len(calendar):
            trading_date = calendar[idx]
        else:
            print("日期超出交易日历范围")
    elif hour < 3:
        # 如果是00:00~02:30之间的时间，交易日是昨天的下一个交易日
        lastday = today - timedelta(days=1)
        idx = calendar.index(lastday) + 1
        if idx < len(calendar):
            trading_date = calendar[idx]
        else:
            print("日期超出交易日历范围")
...
def cook_data(raw_data: pd.DataFrame) -> pd.DataFrame:
    ...
    start_time = (data['date'].min() - timedelta(days=10)).strftime('%Y%m%d')
    end_time = (data['date'].max() + timedelta(days=10)).strftime('%Y%m%d')
    calendar = xtdata.get_trading_dates('SF', start_time=start_time,end_time=end_time, count=-1)
    calendar = pd.to_datetime(calendar, unit='ms') + timedelta(hours=8)
