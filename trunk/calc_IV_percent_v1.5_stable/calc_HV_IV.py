...
def parse_option_code(option_code):
    ...
    if '-' in code:
        ...
        call_pos = temp_code.find('C') # 找不到指定字符串的时候返回 -1
        put_pos = temp_code.find('P')

        if call_pos != -1:
            option_type = 'call'
            underlying = front + temp_code[:call_pos]
            strike = int(temp_code[call_pos + 1:])
        elif put_pos != -1:
            option_type = 'put'
            underlying = front + temp_code[:put_pos]
            strike = int(temp_code[put_pos + 1:])
        else:
            import traceback
            traceback.print_exc()
        # print(f"期权代码: {option_code}，解析结果: option_type={option_type}, underlying={underlying}, strike={strike}")
    ...
    for code in option_code:
        try:
            ...
            if option_price <= 0 or underlying_price <= 0:
                print(f"期权代码: {code} 或标的代码: {underlying_code} 的价格无效，无法计算隐含波动率")
                continue
